#!/usr/bin/env python3
"""
OpenMythos pretraining for 1B model on mixed data (Text + Code).

Single GPU:
    python training/1b_pretrain.py

Multi-GPU:
    torchrun --nproc_per_node=$(python -c "import torch; print(torch.cuda.device_count())") training/1b_pretrain.py
"""
...
class MixedPretrainDataset(IterableDataset):
    """
    Streaming Mixed (Text + Code) loader yielding fixed-length (input, target) pairs.

    Mixed (Text + Code) is trillions of tokens, so `streaming=True` pulls shards on
    ...
    def __init__(self, encoding, seq_len: int, subset: str, rank: int, world_size: int):
        ...
        Args:
            ...
            subset     -- Mixed (Text + Code) config name (e.g. "sample-10BT", "default")
...
def main():
    ...
    Lifecycle:
        ...
        7. Stream Mixed (Text + Code) through grad-accumulation microbatches with
    ...
    # Dataset + DataLoader
    # ------------------------------------------------------------------
    dataset = MixedPretrainDataset(encoding, seq_len, dataset_subset, rank, world_size)
